...
if echo "$BUILD_OUTPUT" | grep -q "built in"; then
  BUILD_SCORE=20
  BUILD_TIME=$(echo "$BUILD_OUTPUT" | perl -nle 'print $1 if /built in ([0-9.]+)/' || echo "?")
...
if [ -f "$COV_FILE" ]; then
  COV_PCT=$(python3 -c "import json,sys; d=json.load(open('$COV_FILE')); print(d['total']['statements']['pct'])" 2>/dev/null || echo "0")
...
LINT_ERRORS=$(echo "$LINT_OUTPUT" | grep -c "error" 2>/dev/null)
