...
if echo "$BUILD_OUTPUT" | grep -q "built in"; then
  BUILD_SCORE=20
  BUILD_TIME=$(echo "$BUILD_OUTPUT" | perl -ne 'print $1 if /built in ([0-9.]+)/' 2>/dev/null || echo "?")
...
TEST_PASSED=$(echo "$TEST_OUTPUT" | perl -ne 'print "$1\n" if /^\s+Tests\s+(\d+) passed/' | head -1 || echo "0")
TEST_FAILED=$(echo "$TEST_OUTPUT" | perl -ne 'print "$1\n" if /^\s+Tests\s+.*?(\d+) failed/' | head -1 || echo "0")
...
if [ -f "$COV_FILE" ]; then
  COV_PCT=$(perl -ne 'print "$1\n" if /"statements":\s*\{[^}]*"pct":\s*([0-9.]+)/' "$COV_FILE" 2>/dev/null | head -1 || echo "0")
...
LINT_ERRORS=$(echo "$LINT_OUTPUT" | grep "error" | wc -l | tr -d ' ')
